...
import SelectMemberModal, {
  SelectMemberModalType,
  SelectMemberOption,
} from '@/components/select-member-modal';
...
import GroupManageModal, {
  GroupManageModalRef,
} from './components/group-manage-modal';
export default (props: {
  ...
  const groupManageModalRef = useRef<GroupManageModalRef>(null);
  ...
  const batchInviteJoin = useCallback(async (gid: string, uids: string[]) => {
    ...
    const sharedSecret = globalThis.wallet.signingKey
      .computeSharedSecret(Buffer.from(group.pub.substring(2), 'hex'))
      .substring(4);
    ...
    for (let index = 0; index < users.length; index++) {
      ...
      const userSharedSecret = groupWallet.signingKey
        .computeSharedSecret(Buffer.from(user.pub_key.substring(2), 'hex'))
        .substring(4);
    ...
    <View
      ...
      <View
        ...
        <View
          ...
          <MemberItem
            avatar={require('@/assets/icons/circle-plus-big-white.svg')}
            onPress={async () => {
          ...
          />
          <MemberItem
            avatar={require('@/assets/icons/circle-sub-big-white.svg')}
            onPress={() => {
              console.log('踢出用户');
              if (props.members) {
                const authId = globalThis.wallet?.address.toLowerCase();
                const options: SelectMemberOption[] = props.members.map(
                  item => {
      ...
      <MenuItem
        ...
        icon={require('@/assets/icons/qrcode.svg')}
        label="群二维码"
      />
      <MenuItem
        label="群容量"
      ...
      <MenuItem
        onPress={() => {
          ...
          groupManageModalRef.current?.open();
        }}
        icon={require('@/assets/icons/arrow-right-gray.svg')}
        label="群管理"
      />
      <MenuItem
        onPress={() => {
